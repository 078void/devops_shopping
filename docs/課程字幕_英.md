...
**Duration: 2 minutes**
In this video, we are going to focus on Azure Pipelines.

Azure Pipeline is a Continuous Integration and Continuous Deployment (CI/CD) tool provided by Azure under Azure DevOps

umbrella. With Azure Pipelines you can build, test, and deploy your applications across all languages.

Azure Pipelines can pull your source code from various repositories and execute a build according to

your specifications.

Let me give more detail about what is Continuous Integration and Continuous Deployment (CI/CD) flows.

This flow provide to Automate tests, builds, and delivery. Continuous integration automates tests and builds for your project.

four your project.

Continuous integration helps to catch bugs or issues early in the development cycle, when they're easier
...
and applications, Automated release pipelines consume these artifacts to release new versions.

We can provide this flows with creating a pipeline on azure pipelines.

We will define our pipeline in a YAML file called azure-pipelines.yml with for the one application.

But of course we will create pipelines as per microservices ci/cd flows. Pipelines are stored with our code,

with inside of our codebase.

so we will push and commit pipeline yaml files like our code strucure on github.

on github.

By this way, we get validation of your changes through code reviews in pull requests.

Any change to the build process might cause a break or result in an unexpected outcome.

Because the change is in version control with the rest of our codebase, we can more easily identify the issue.
## 10-4 Deploy to AKS through Azure CI/CD Pipelines
**Duration: 3 minutes**
In this section, Our main target is automate deployments of Shopping Microservices into AKS

with using Azure CI/CD Pipelines.

So before we start let's remember what steps we have done when manually deploying to AKS.

As you can see, that we have created docker images for our microservices. Also we compose docker containers

and tested them.

We deployed these docker container images on local kubernetes clusters, and pushing our image to

the ACR , shifting deployment to the cloud Azure kubernetes services (AKS), and update microservices

update microservices with zero-downtime deployments.

So now it is time to check that what steps we are going to do when automate deployments into AKS

with using Azure CI/CD Pipelines.

There are several steps we need to go through to set up Azure Pipelines.

We will develop our project on visual studio. Storing our codes on Github Repository. Azure pipelines connect

and triggering with Git repo changes.
...
Here you can find example scenario that explains automation deployments into AKS

with using Azure CI/CD Pipelines.

Whenever we pushed any new code or update any existing code on Github, Azure Pipeline will pull the changes

and start the automation.

First, it builds a new version of docker image from docker file which means creating a container image

from our application.

After this step, pipeline will hit docker push command which will push to your docker image

to an Azure Container Registry(ACR) with the new version tag name of image,

After that, Azure k8s cluster will pull the image of the application and it creates and run a container

run a container from this new image inside of the k8s pods,

AKS will perform rollout deployment that pods replace one by one with the new version of image.

So end user never effect to these zero-downtime deployments.

By this way, we can deploy hundreds of deployment without affecting any system in a daily time.

Also same as the rollback deployments will be no effect.

So we explained the basics.
...
講座縮圖
0:01 / 4:56
In this video, we are going to sign up for Azure Pipelines, Azure DevOps.

Let's say connection.

In order to work with Azure Pipelines, we need to create an organization first and then create a project

inside of the organization.

But before that, you should sign up for the Azure DevOps Organization and Azure Pipelines to begin

managing CI CD to deploy your code with pipelines.

So let's start with the sign up with the Azure DevOps.

Azure Pipelines.

We can sign up with a personal Microsoft account.

If you have a Microsoft account, you can sign up for the Azure DevOps.

So go to this URL.

Microsoft Azure.

Azure DevOps, and you can just click the Start free.

And after clicking after click to start today, you will see the sum form will be open.

You should open an email address, provide your email address, phone number or Skype ID.

Actually, you should provide the Microsoft account and continue and finish to this login process for

the Azure DevOps.

So let me log in, just click the start.

So I have already login before and created a Azure DevOps free account.

So that's why directly it is logging for me.

But you should see the Azure DevOps account and see the login Azure DevOps portal.

After that you will reach this URL and these information dev.azure.com address.

And this is my username and these are the my organizations.

So what we have said before, we should set that before we start, we should create a new organization

and inside of that we should create a new project.

So for me, I have an organization with my username and now I'm going to create a new project.

When you finish this process login process, you should create a new project in order to use Azure DevOps

process.

So let me click new project.

The project name will be the shopping and we are storing.

And automating are swapping microservices.

And I'm using the private one.

Not required the public one.

So let's hit the create button.

And see what?

Seven.

This will be the create a project under our organization.

So please look at the URL.

This is my organization.

This is my project.

So you should also similar.

Your definition should be in here.

So as you can see that now we can see the Azure DevOps applications like Azure boards, repos, pipelines

and test plans and artifacts.

So after you log in and create the project, you will see this kind of menu inside of the Azure DevOps.

So let me show you some things about the boards.

Just click the boards and you will see this kind of window and we can create a new item.

Let me create a new item.

New, for example, new epic.

You can show the create a pic and issues and attach to development items to commit.

So for example, deploy microservices.

This could be our epic and you can set the state reason and put additional details in here.

The very good feature of these boards, you can directly attach these items task with your development

tasks and you can commit numbers.

This is very good feature and once we are creating an issue, let me go back to items, for example,

if we are creating an issue.

And set it.

Create pipeline for Shopping API.

You can think about this as a task and you can attach the development items with your commits in here.

By this way, you can very easily track your issues and in which issue handle in which commit in your

GitHub repository.

So this is an example of Agile methodology implementation.

You can follow your team outputs in here with a scrum methodology.

So as you can see that we have joined the Azure DevOps and see benefits on the web portal.

In the next video we will focus on the pipelines.
## 10-6 Create Our first pipeline with Azure Pipelines
**Duration: 7 minutes**
In this video, we are going to create our first pipeline with using Azure Pipelines to build a GitHub

repository.

Let's take an action.

Before we start, let me recap our operations.

We have GitHub repository and we have developed our microservices on that repository and also we have

created an Azure DevOps organization and we have created project on Azure DevOps.

So now we are going to create our first pipeline.

Sign in to Azure DevOps organization and navigate to your project.

So we have signed in and this is our newly created project.

We have navigated to our project.

So go to the pipeline section.

Click the pipelines and we are going to create our first pipeline.

Just hit the Create pipeline button.

And as you can see, that we are this is the visit of the great pipeline operation.

The first step is the giving the code repository.

So in our project we are going to navigate the pipelines and choose the action to create new pipeline.

Now we will give the repository as a GitHub or project stored in the GitHub.

Okay for you.

I think you will walk through these steps of the visit by first selecting the GitHub and as the location

of your source code, it will.

When you make it first time it will redirect to GitHub to sign in.

If so, enter your GitHub credentials in order to reach your repository.

After that, when the list of the repositories appears, select our shopping microservice repository

for my case, I have a.

A minute run repository.

But now.

Let me see all the repositories.

Okay.

I have created a net run organization, and in our case, this is our repository.

ASP.Net Run.

Run DevOps.

Let me click this repository.

Okay, Now I'm granting this application.
...
We are providing GitHub.

We are providing GitHub password and we are granting our pipelines.

These are Microsoft account.

This will take some time.

Okay.

We have now comes to configure step.

Now the repository can be reachable from our Azure DevOps project.

So now we will reach the configure your pipeline step.

So as you can see that there are several template tasks in here.

These are the basic pre-configured task which we can use one by one.

And this is the creating pipeline for us.

For example, we are going to start with the Docker.

Docker, build a Docker image and another task predefined task is Docker build and push an image to

Azure Container Registry.

And as you can see that this is deployed to Azure Kubernetes services with pushing the Azure Container

Registry.

This is the including the Docker task and also deploying the Azure Kubernetes services.

So this is what we are looking for.

But I am going to start with the one by one.

So that's why I will start with the build and push the image to Azure Container Registry.

That means I am starting with the continuous integration part CI part.

This is the including the CD part.

Deployment part.

This is the continuous development part, but I will show you one by one.

So that's why select the Docker build and push an image to Azure Container Registry.

Basically, this task covered our first part of deployment process.

We will start with build task, which is the Docker build and push.

Also, we can see that there is a Kubernetes deployment which contains the curb and push template.

We will see it later when we explain the deployment stage.

So let's configure our task.

Of course, we need to provide our information.

Let me sign in.

We are going to give access for our Azure subscription.

So as you can see that the second step is container registry.

Select the container registry.

We have one container registry shopping, ECR and the image name.

Now it is creating predefined and looking for the Docker file.

It is.

Find the Docker file under the shopping API.

It is looking the first local file in our project and find this docker file.

Okay.

I am not changing anything for now.

Let's validate and configure button.

Now it is configuring resource and generating Yaml.

So as you can see that every pipeline finished with the generating Yaml.

So in here we have a Azure pipelines yaml now under the run DevOps GitHub repository.

So we have some of the pipeline commands in here.

I will explain later.

But you can think now we have configured one Docker task and this task will be with our image and push

to our image into our shopping ECR Azure Container Registry and it will provide with the adding our

repository.

Azure Pipelines Yaml file.

So as you can see that it is generated Yaml file which includes our task.

In the next video we will examine this file.
## 10-7 Pipeline Docker Task Yaml File Explained in Azure Pipelines
**Duration: 11 minutes**
In this video we are going to explain our pipeline Docker task yaml file and explained in the Azure

pipelines.

Let's take an action.

Now let me check the generated Yaml file.

As you know that in the last video we have generated this Azure pipeline Yaml file with the Docker task

and now we will review our pipeline yaml file.

So see the general file and examine one by one.

As you can set, it will start with the comment section.

This is the template Docker task.

So it will including build and push an image to other container registry inside of this pipeline Yaml

file and every pipeline.

Start with the trigger keyword and if you scroll top on this trigger you can see the explanation.

Continuous integration triggers.

So this is for when we start our pipeline definitions.

That means it starts any change on main branch of our GitHub repository.

So this is the branch name of our GitHub repository.

So whenever we push any code into our repository, this pipeline will be triggered.

Okay.

So there is a resource definition.

This is default definition of resources.

I'm not going detail and variables.

Variables are very important part.

You can think about the variables like a normal development and you can think this Yaml file as a normal

code window.

We can define variables on top of our page and use these variables into our task objects.

So for example, we have image repository container registry variables.

We will use these variables as a parameter inside of the tasks.

So let me explain one by one in which variables we are going to use.

The first parameter is the registry service connection, and the explanation is the container registry

service connection established during the pipeline creation.

Maybe you remember when we add this task.

Into a template.

In the select section, we have a configure pipelines and configure task.

When we are configuring to task, we are providing to our Azure subscription.

So that means this is the unique connection identifier.

We when we configure our Docker registry, it will basically connect our Azure subscription and connect

our Azure Container registry into that subscription.

So this is the connection of that.

Let me show more clear with this Docker connection is created when we have added the task.

Let me show you the settings.

If you right click the project settings and open new tab, let me show you the project settings.

Under the pipelines.

You can see the these general settings, port settings and the pipeline settings.

We are configuring the pipeline settings if you check the source connections.

You can see the two service connection, one for the GitHub.

These are GitHub.

Repository connection by this way are generated Yaml file can identify when we change in our repository

and if we push any code into the repository, this pipeline will be triggered by this connection.

And the second connection is shopping.

ECR shopping.

ECR is the our Azure Container Registry into our Azure subscription.

So we have provide the Azure subscription information when we are configuring this task and these are

service connections and saving into our project settings under the service connection.

And this will have a unique identifier and we are going to use these unique identifier parameter into

our pipeline variables.

So as you can see that Azure pipelines save our connections in here and we use these connections during

the pipelines.

Also, Azure Pipelines create environment for us under the pipelines.

It will create environment for us and it will set that these connections related to these environments.

But for now, I'm not going to show environments.

We will check in the next videos, so let's focus on the explanation.

Our Yaml file.

So there is to another variable image repository and the container repository.

These two variables are Azure container registry required parameters in order to push our Docker image

to the Azure Container Registry.

Maybe you remember these are login name of Azure Container registry and this is the image repository.

We will see later.

We should refactor and change these information.

There is no image like that, but for now it is auto generated.

I am not changing anything.

So let me.

Scroll down and see the Docker file pit.

This is Docker file Pit which using for the building Docker image.

If you see the volume, this is indicating our Docker file looking for the Docker Docker file for the

shopping API and it will try to build the image from this Docker file into our GitHub repository.

And another one is the tag.

This is the.

Take number what we are going to pipeline going to create a tag for this image.

So as you can see that there is a predefined.

Class and predefined built method.

This is for the unique tag name, which is a predefined class that we can build or build number in our

pipelines.

You can give the build number or you can use the build ID.

It will generate custom newly created ID for us for every pipeline.

So this will be auto generated classes and auto generated methods.

You can think about that.

We have some predefined values can be used during the pipelines.

We will see in the later.

But you can think about that.

It is generating auto numbers for us.

Build numbers.

Okay.

We have finished two variables and there is a VM image name.

This is the agent VM image name Azure Pipeline Virtual machine image.
...
We have now in this stage section starting the states states mostly separating built and deployed states.

For now, we have only built state because we didn't specify any deployed state set into this states.

We have jobs and into the jobs we have tasks.

It is a three structure stage job and the task.

So let me see our example.

As you can see that we have a built state for now.

And this will with decrement and push to the image to the Azure Container Registry and the job definition

also built.

And we have a VM name and the important part is the task.

So let me say the task name is the correct two.

If you scroll this Docker on Docker keyword, you can see the explanation with or push the image.

So this is pre-built Azure pipeline task that provide the build and push the image to the container

registry.

We can examine task and input parameters from official documentation.

I will show you in the next videos.

But now let's go inside these parameters.

So under the input input text, you can see some of the inputs in here.

So as you can see that this is the input part and we use the parameters.

Which indicates the ACR and command for the build and publish.

You can think about the inputs are method parameters.

If we can say that task is a method, inputs are the method parameters and the first parameter is the

command, and the command is the build and push repository.

We have defined in the variable section.

This is the image repository.

The file also we have defined in the variable section.

By this way we can use the variable in this format.

So container registry.

Also, we have defined the service connection of the Azure Container registry and the tag number including

in here, everything seems good.

So we have understand what is the basics of the pipelines.

So now we are going to save and run our pipeline.

If you click this button, this will be prepared for us commit message because this is going to commit.

A new comment for our repository.

It is adding the Azure pipelines Yaml file and at the same time creating a pipeline and running the

pipeline after the committing code.

Okay.

Our pipeline is started.

Once we save this pipeline, it will push Azure pipeline Yaml file into our GitHub repository.

You can check the GitHub also and pull this code from our Visual Studio code.

So now let me see the portal for watching this build.

If you click the pipelines, you are seeing that this pipeline is running for us now.

If you click the details.

You can follow the world stage and see the logs in here.

And these are the steps.

Now it is performing, build and push task.

Okay.

Yes, we got the exception.

If we check the exception in here, we have exception in the seventh stage.

And there's a problem with the our Docker operation.

As you can see that we get error regarding the Docker file copy command.

It can't find the Docker file correctly.

In the next video we will examine this error.
## 10-8 Pipeline Docker Task Error Fixed in Azure Pipelines
**Duration: 17 minutes**
In this video.

We are going to fix our pipeline Docker task error.

Let's take an action.

But before we start, I would like to clarify this create pipeline operation.

Microservices are independent deployable units, so pipelines should be separate.

For this reason, Azure pipeline templates also created for one microservices.

Actually we should create pipelines for every microservices.

But now the idea is that even we have three microservices, we will start to optimize shopping API microservices.

We will write pipeline for the shopping API.

Now after that, we will continue with the shopping client microservices.

But now let's focus on the creating pipeline for the shopping API.

As a summary, now we are starting our pipeline for shopping API microservices.

Okay.

Let me come back to our error in the pipeline.

Go back in here and go to Azure DevOps portal and open your pipeline.

And in the build and push step, we have got an error.

As you remember, we have got this error when building our pipeline.

If you check this error in this line, copy failed.

You can see that.

Despite the curb wheeled shopping API shopping API project.

No such file or directory.

So this is can find our docker file in our repository.

The problem is the docker file path.

So as you know that our Docker file is more deeper level of the folder structure.

So we should give the build context of the Docker build path of the pipeline.

So let me show you our.

Repository.

Let's go to the repository and run DevOps and see that.

This is our project and shopping API and Docker file includes in this level.

But somehow this.

Pipeline cannot find our.

Local file path.

For that reason, we should define build context of the Docker file with context.

Provide that.

Please check this file path when you are looking for the Docker file.

So that's why it is very important.

We should define the file path with context in our pipeline.

So let me show you in the our GitHub repository.

Also, when you save and run this pipeline, as you can see that our pipeline Yaml file pushed from

the Azure pipeline portal with the description to setup with the Azure pipelines.

Okay.

Now before editing our pipeline Yaml file, I would like to show you in the pipeline section.

As you can see that in the pipeline section we saw the our pipeline which is named as run Run DevOps.

If you click this pipeline, it is now show the runs.

So this is our latest run.

If you have several run run profile, you can see list of the run profiles.

So let's click this one and you will see as a summary of the latest run.

So repository is in here and the version and this is the branch so you can see details and in the error

text, you can see the details of the errors.

So you can click the build or the error tabs.

It is directly go to the running log file.

You can see the logs very clearly in here.

So it is very easy to use Azure pipelines from the dashboard and you can locate the error by clicking

the this step.

Okay.

Now now we clarify this error and we understand that we should set the our build context when we are

defining our pipeline.

So for that reason, we should go to the Visual Studio code, open the Visual Studio code.

Let me close this one.

But in here, as you can see that we have no any other pipeline symbol because these Azure pipelines

Yaml file push from the Azure DevOps portal.

In order to get this file we should go to the source control.

And let me pull latest code from the, our main branch of the run DevOps repository on GitHub.

So the operation complete and now I can reach the Azure pipelines Yaml file.

I saw this file, we examine this file one by one in the last video.

So this is the trigger.

One main branch changes anything.

This pipeline will be trigger and variables.

We can see set of variables in here in this stage.

For now we are developing continuous integration and only with stage existing in our pipeline Yaml and

in the inputs we are going to use task Docker add to and we have some input levels.

So we should add build context in somewhere in when creating image and looking for the Docker file in

the Docker add to task.

So in order to do that I am going to add a build context under the container registry.

Let me add build context.

As you can see that we have a IntelliSense.

So in the build context I would like to use.

First of all the source directory and adding the shopping.

Shopping.

So let me compare with my GitHub repository in GitHub repository.

I'm starting with the shopping.

So.

These will be located my GitHub repository.
...
These parameters locate to my GitHub repository, which is the run DevOps, and also I'm providing the

shopping because we have.

Microservices, including the under shopping folder.

After that, the Docker can be reached the Docker file.

So this is the our main step to solve our pipeline error.

So we define two build contexts.

Pet Normally it search for the root pet, but now we specify our project pet in order to find correct

Docker file.

Okay, let me see the documentation.

I would like to also see the documentation.

You can ask to me how you can find the build context object and where you know that these parameters

there is a official documentation in Microsoft, we can see the task with tasks under the pipeline tasks,

also DevOps, other pipelines, pipeline task and the task.

We have a Docker task.

We are going to use this task and in the Docker task, if we search for the build context, you can

find the all inputs in here and we are going to provide build context.

It is optional, but we are giving the path to build context.

The default value is looking for the root folder.

So in the root folder we have no any Docker file.

That's why we are specifying the build context.

It is good to check official documentation when you are using the any task in the pipeline and also

parameters.

It is very good explanations in the official documentation.

But also I.

I will mention about this task and the official documentation in the upcoming videos.

So now we have finished to solving operation or pipeline error.

Now we should somehow run our pipeline again, right?

You can run this pipeline in the dashboard, but I would like to use Visual Studio code because as you

know, that this pipeline triggering when any changes happen in the main branch.

So we are in the main branch and we are going to change this Yaml file because we are adding the build

context.

So if I push any this code into the my GitHub repository, this pipeline should be triggered automatically.

So let me try this operation.

Come here.

And say that we context edit.

Commit our changes.

And I am pushing these changes to the GitHub in order to run pipeline.

It is enough to push this code to the GitHub.

Other pipelines identify these changes and trigger to run run pipeline.

Lets me push the code and see the pipelines we are going to commit.

And this is the create commit on my main branch at that file.

So one main branch changed.

You can see that in here the main branch changed.

It will trigger automation.

Let's check the our automation pipelines.

Okay, now you can check the pipeline status.

When you click the pipelines from Azure DevOps, under the pipelines, you can see that we have success.

The latest run can be get the success response.

So if you open this pipeline, you will see that we have second run profile triggering from the main

branch.

When we are pushing the code, which name is build context edit command.

So we identify that these run profiles will be different according to in which code we are pushing the

GitHub repository.

Let me open this build profile.

You can see that it is triggering by our username, which is the GitHub username and the repository

is in here.

Main branch we have.

Using and if you check the jobs in the boot section.

You can see that now the old steps are green, so that means all steps are working fine.

The error was in build and push stage step.

If you can see the width and push step, you can see that the creating image.

Is performing successfully because this time it can be find the Docker file.

So it is building the image from our shopping API Docker file.

And after that, take this image with the build number and after that, push this image to other container

registry.

So in order to verify this.

Image.

The crewmates.

Let me go to the Azure portal.

Go to Azure Portal in the resource groups.

Let me go to open my resource group.

So open the Azure Container Registry.
...
And open the shopping API repository.

So we have pushed the code from the shopping API repository.

Let me refresh.

Let me check the shopping client.

Okay, I understand the problem.

So please check the repositories.

As you can see that it is creating a new repository because we didn't change the our Docker image name.

The name was staying and run DevOps.

So that's why it is creating a new Docker image with this name.

And also it is tagging with the build ID build number ID.

So in order to solve this problem we should go to the our input parameters.

Let me open Azure pipelines in here in the variables, you can find the image repository.

We should change this image repository as a shopping API by this way.

Once it is pushing the image to the our Azure, it is looking to the existing image.

If there is a shopping API, then it will create a new tag with the number of the 45.

This is the build number.

This comes from the this tag definition with that built ID, so at least we can see the successful build

continuous integration part.

But of course we should extend these operations with the shopping client.

So as you can see that we have successfully optimized build and push the Docker image to ECR.

When we have committing to main branch of the GitHub repository.

So continuing continuous integration part is almost done.

So let me fix this error and see what's happened.

I would like to fix with you in here shopping API.
...
If I push discount shopping api image name.

Let me commit and push.

Let me trigger again this pipeline.

And see that what happened this time?

I would like to expect in the Azure Container Registry there will be.

46 number take should be including under the shopping API repository.
...
Shopping API image name commit came in the GitHub so we can check the our pipelines now.

New pipeline is started.

So new run is started.

Let me open this run and watch these operations with together.

So this is now build and push step.

We already defined the build context and we expect that this will be pushed to the limit, built and

created a commit and push the commit to Azure Container registry under the shopping API repository with

adding new tag number.

Tag number should be the build ID, so that means we expect it 4 to 6 now.

So maybe you remember this step was getting error before the seventh step, but now it is working fine

because Docker file is detected correctly.

Okay.

Now boiling image.

This is the file steps.

Yes.

Now, pushing the image.

You can follow the looks in here.

Okay.

Or with operation succeeded.

You can see the new run profile started with this checking operation commit operation from GitHub.

So now if I check the my shopping API.

Yes.

When I refresh this shopping API repository in the Azure Container registry, which name is shopping

ECR, you can see that new tag created in here.

As you can see that we have successfully optimized, build and push the image to the Azure Container

registry when committing to main branch of our GitHub repository.

So continuous integration part is done.

In the next video, we will focus on the continuous delivery.

That means deploying to Azure Kubernetes services.
## 10-9 Pipeline Tasks and How to Decide to Write Correct Task in Azure Pipelines
**Duration: 4 minutes**
In this video, we are going to check pipeline tasks and how to decide to write correct tasks in our

Azure pipelines.

Let's take an action before explanation.

Let me add a new step in our pipeline definition.

So open the Azure pipelines and go to pipeline section.

See that this is our pipeline.

So I'm going to edit this pipeline in order to add a new step.

Click here and click the edit button.

And so now.

We have in the Azure pipeline single file.

As you can see that this is the our pipeline Yaml file.

So after the build operation I would like to deploy my application with using this Docker image.

So what, what should I do for now?

So as you can see that we have seen that there is a taskbar, right?

So where these jobs comes from, these jobs are and the task comes from the predefined from the Azure

DevOps.

So let me give some example.

If I would like to deploy to Kubernetes, you can click the Kubernetes task and of course you should

provide these parameters.

For example, Kubernetes service connection.

You should connect with your Kubernetes services.

In our case, we should connect with the Azure Kubernetes services and communicate with the allocate

with these Azure resources before that.

So now we have no definition.

So let me click that and see that.

In the last stop, we can see the new task which name is Kubernetes Manifest Task at zero.

And it is getting some input variables right?

So let me think about that.

We I have another task, which is the again, Kubernetes.

But this time I would like to run some Kubectl commands.

In order to do that, let's click the kubectl and it's again expecting lots of parameters.

Let me click the add and you can see that we have a Kubernetes task at one and a several input variables,

right?

It is expecting several input variables.

So where these jobs and the task comes from and how you identify which task you should use, which parameters

you should use, and so on.

So in order to understand that things, let me don't say this anything.

Leave this page.

And Microsoft has great documentation for other pipeline tasks.

Please go to the this URL in the documentation.

Azure DevOps.

Azure Pipelines Pipeline Task and Task Index.

You can find all tasks into the Azure pipelines and also including all explanations.

For example, for our cases we are looking for the Kubernetes.

So this is the kubectl task.

This is the Kubernetes manifest task.

If you open the Kubernetes manifest task.

You can see the details and input parameters, deployment actions.

The whole information storing in this documentation and also is an example usage of the Kubernetes manifest

task.

So that's why we can check from the tasks from the official document and also identify in which parameter

you are going to use and why you are going to use.

These are the getting information from the official documentation.

Okay.

As you can see that we can able to see tasks and requires input parameters with the explanation.

You can read this task before creating your own pipeline.

I will share the links in this video.

So now for our project, it is good to write our own pipeline from the beginning.

So in the next video we will focus on the continuous delivery part.

That means deploying Azure Kubernetes services.



## 10-10 Create Pipeline for Continues Delivery with Deploy to AKS Task
**Duration: 6 minutes**
In this video, we are going to create pipeline for continuous delivery with deploying Azure Kubernetes

Services task.

Let's take an action.

Before we start, we should remove existing pipeline yaml.

So let me click in here and delete existing pipeline.

And it is required to name.

Let me copy and paste and delete because we are starting from scratch from the beginning.

So I would like to be everything is clean.

It's the same way.

I'm going to remove these Azure pipelines, HTML file, delete this file and.
...
I'm pushing the quotes.

On GitHub.

We are going to start the beginning.

That's why I am deleting all files.

Yes.

Now our application is cleared.

Also, my pipeline section is clear.

So now we can create fresh pipeline for our deployment case.

Let's hit the create pipeline button and select the GitHub.

Give our repository, which is the ASP.Net Run Run DevOps.

And now it's time to configure our pipelines.


## 10-11 Create Pipeline for Continues Delivery with Deploy to AKS Task Part 2
**Duration: 9 minutes**
In this video we are going to examine and configure.

Yaml file for continuous delivery with deploying Azure Kubernetes Services task.

Let's take an action.

As you remember in the last video we have generated this Yaml file.

Azure pipelines Yaml file and this will include it configured for the deploying Azure Kubernetes services

which also including the build and pushed image.

Azure Container Registry task.

So let's examine one by one again.

The trigger we already seen this.

It is this pipeline will be triggered when any change in the main branch on GitHub.

So variable section, we already saw that all these variables were explained in the last video, but

this is the new image pull secret image.

Pull secret for pulling images from the Azure Container Registry.

As you remember, we have also created these image pull secret.

It is mandatory for pulling image Azure Container Registry from Azure Kubernetes Services.

Actually, it is the best practice when you are pulling images from any Kubernetes services, from the

any container registry, you should create an image pull secret in order to make securely these operations.

Okay.

The VM image name is the Linux based operating system.

And now we come to the States section.

This time we have two states, one for build, one for deployment.

Build state is almost exactly same what we have done before, but we need to add build context again

in here.

I will make changes in the next video, but these steps are the same state build, jump in here and

also steps in here.

We are using the Docker add to task and this task basically perform, build and push the image to the

Azure Container Registry.

We are using the input parameters as a image repository Docker file path for the our Azure resources

into the Azure Container Registry.

These parameters indicated to us our environment.

So this is the boost state which stays almost the same.

But let's see that there is a new thing comes in here upload, manifest artifact manifests.

What does this mean?

So we have seen this newly.

We have seen this very newly upload and artifacts.

This is very important because this place is the connection point of build and deployment states.

You can also identify that we finished the boost stage and we are starting the deployment stage.

Between these stages we have upload an artifact definitions.

After we decrements, we upload some additional files in order to use from the deployment stage.

You can think about these task running different machines on cloud.

And once we finished it, upload items into one place that deploy task, read and use these files when

deploying applications.

We will also can see and download these uploaded artifacts on Azure pipelines.

We will see details later.

But for now you can think about these definitions for moving some additional files to the deployment

stage in order to use in in the deployment stage.

So maybe you understand from the name manifest, these will be the Kubernetes manifest files.

Okay.

Let's continue with the deployment stage.

This is the second stage after which stage we have a deployment stage.

And in the deployment stage, we have some additional parameters.

You can see that depends on in the world stage.

That means this stage will not run when Wilt is completed and we have job definitions in here.

And under the job definition, we have deployment strategy.

Okay.

Basically, this is the deployment job.

It is specific for the deployments.

So it includes deployment.

Strategy is run once, but we can modify later blue, green, canary and roll out deployments.

But for now, Azure Pipelines support the run once deployment strategy.

Basically it creates image pull secret for the pulling image to Azure Container Registry.

After that, running manifest files into the AKs cluster.

This is the Kubernetes task.

It creates a image pull secret for pulling image to ECR and after that, running manifest file into

the AKs cluster.

So as you can see that these manifest files.

In here.

You can see the manifest files.

Let me see that.

The first step is creating image pull secret.

And the second task is that the first task is the Kubernetes create image.

PostSecret and the other task is to deploy to Kubernetes cluster.

So in the deploy to Kubernetes task, you can see the manifest files.

And these manifest files comes from the manifest folder and looking for the deployment Yaml and service

HTML file.

So as you can see that these manifests looking for the manifest folder which we uploaded in the world

stage and looking deployment and service HTML file for the Kubernetes deployments.

So you remember these manifest file name into the.

These upload section we have uploaded manifests folder, so that's why it can be used in here.

In order to reach deployment and service emails and this task, perform that reading these Yaml files

and applying these Yaml files onto the targeted Kubernetes cluster which which for us is the Azure Kubernetes

services.

So you can see that our deployment stage have two tasks.

So you can see that our deployment stage has two task one for the create image pull secret, and second

for the deployed Kubernetes cluster.

We have created secrets in order to pulling image from the Azure Container Registry.

And after that we are reaching the manifest folder and reading deployment and service Yaml file and

applying these service symbol file to the target Azure Kubernetes services.

So of course we are providing the image pull secret information, container registry and image register

and tag information.

These variables come from the variables section.

By the way, we have prepared this task for only one microservices.

You can see in here in the variable section.

As you can see that we have image repository shopping API and this is for online shopping API.

But we have more microservices, so we will cover all of them in the next video.

These pipelines should be separate as per microservices, but now let's continue focusing of the shopping

API with image and deployment microservices to Azure Kubernetes services.

We should separate pipelines as per microservices in the upcoming videos, but let's continue this way.

So now I am going to save and run this pipeline.

If you click the Save and Run, this is the commit message and.

See that there is also additional two files, one for the this pipeline file we already seen before,

but this time it is creating a manifest folder.

And in this folder it is adding the deployment and service HTML file which basically expect to us adding

these deployment Yaml files or service definitions into these HTML files.

By this way in the deploy task can able to get this folder can be able to reach and apply these yml

file into the Azure Kubernetes services if you click the Save and Run.

You see that these.

Yaml files will be committed in our GitHub repository.

Okay.

Pipeline creating.

This will be get the error because we didn't fix the Docker pit.

We should put to the build context.

But let me show you in here.

If you check the GitHub repository, you can see the manifest file created in here.

And also we have other pipelines in the manifest file.

We have two Yaml files, deployment and service and our pipeline will be get the error due to cannot

reach the build context.

In the next video we are going to refactor this generated yaml file.
## 10-12 Refactoring Our Pipeline for Continues Delivery with Deploy to AKS
**Duration: 8 minutes**
In this video, we are going to refactor our pipeline for continuous delivery with deploying Azure Kubernetes

services.

Let's take an action.

As you can see that we already expected this error.

If you open the pipeline and see the run profile in the build stage.

We got the error.

And if we check this error when pushing the quotes, it cannot find the Docker file in the correct location.

So we know this error.

We should provide the Docker build context.

So let's put the context into the Azure pipelines.

But before we start, let me pull the latest code because this will be adding new files when we have

created this pipeline.

Although pipelines and the manifest files open, the Visual Studio, Visual Studio Code and come to

source control and pull latest code from the GitHub.

And once this bar finished, we will see that Azure Pipelines and the manifest folder came successfully.

As you can see that we have manifest folder and it is including deployment and service definitions.

This will generate it from the our Azure pipelines and these will configured by the also Azure pipelines.

As you can see, it is configured or shopping API, but of course it is not correct.

For example, image name not this way or.

We should also including additional.

Uh, parameters.

For example, shopping load balancer.

It is not correct for our case.

Okay.

But we saw that we have a manifest file also Azure Pipelines, HTML file, which is our pipeline.

So now we can start with refactoring this file and the manifest files.

So let me start with the fixing Docker file path issue.

In order to do that, we should go to the Docker push task and under the container we are going to.

At the world context.

This will be the world context and the.

That should be the shopping.

Okay.

Because we compare with the GitHub, we have a shopping folder.

Okay.

So I have added these build contexts into my Azure pipelines HTML file.

And now we should fix the deployment part, right?

I think the build part is finished and will be work what we have expected.

But now we should fix the deployment part.

As you remember, we have set that after boot stage completed.

It is created artifact files.

And upload it into the manifest folder.

So these two lines of codes after we completed.

It is created artifact files and uploaded into the manifest files.

So Deployment State is looking for the manifest folder manifest folder and read to deployment files

for running on Azure Kubernetes service deployments.

So see this line of codes?

This is so important step because once we've completed this file, should be filled with the Kubernetes

config files.

You can check in the manifest files, deployment and service Yaml and deploy part will be read these

and perform deploying the Azure Kubernetes service with using these manifest Yaml files.

According to this, let me refactor the Yaml file name when deploy stage looking at this artifact.

So let's come to the deployment task to the Kubernetes and under the manifest definition, let me modify

that.

This should be look for the shopping api yaml file.

I would like to this yaml notation and I'm going to remove this one.

Okay, so we set that.

We will have a shopping yaml file under the manifest folder and please check the shopping yaml file

in order to deploy shopping api.

Microservice.

This should be the shopping API.

I'm sorry, shopping API.

Yaml file because we are working for one microservice which is shopping API and we have one yaml file

for the all microservices.

So that's why I am modifying this by removing deployment and service yaml file.

So we said that we are looking for the shopping Yaml file instead of two files.

Please look to shopping yaml file.

So now we should put the two Yaml files into the shopping yaml file.

In order to do that we should add the deployment and service configurations into shopping yaml file.

And let me.

Create a new file under the manifest.

Which is a shopping yaml file.

And this will include our.

Dropping Yaml file deployment and service definitions.

So we have already defined before in the folder.

If you check for this file under the shipping Yaml file we have already deployment definition and the

service definition included for the shopping API.

So I am copy these lines and I am pasting under the manifest folder shopping api yaml file.

This will contains deployment configuration also service configuration and.

So let me Now we can delete this unnecessary deployment and yaml files.

So now our manifest files folder only including shopping api yaml file and also we arrange our pipeline

yaml file according to check shopping api yaml file under the manifest folder with giving this definition.

Okay.

Please save all files and we are going to check in our code when we are committing these codes.

The new pipeline will be start because this pipeline will be triggering one main branch.

Have any changes.

So I'm going to.

Come here and say that with context and manifest change.

Let's commit and push the code.

But once we push the code, this will triggering a new pipeline into Azure DevOps, Azure Pipelines.
...
Now let's check our commit in the GitHub.

We solve that commit came in here.

So in the pipelines we can come here.

And as you can see, that pipeline is started.

Well, let me open the pipeline and see the world stage.

We will.

We will see new run starts and we commit our pipeline.

Let's check in the codes in the trigger the pipeline.

We will examine the results in the next video.
...
**Duration: 11 minutes**
In the last video we have successfully deployed and automated shopping API deployment on the Azure Kubernetes

service.

In this video we are going to manage pipelines for Multi-container microservices with CI CD flops in

Azure Pipelines.

We have developed multi-container microservice application, so we should have separate pipelines for

microservices.

We should create a new pipeline for shopping client application.

Let's take an action.

First of all, we should start with managing the folder structures.

As you remember that in the last video.

I have.

A manifest file manifest folder and under the manifest folder we have a shopping API yml file and I'm

copying this shopping api yaml file from the folder.

So if these two files are identical, why we need to manifest files.

We will fix this problem in this video.

Okay.

We are going to create two pipelines for two microservices and we are going to trigger these pipelines.

Only related microservices is changed.

So let me create a new folder.

Which name is Pipelines?

But this is not should be under the case.

Let me delete this one.

And.

Create a new folder, which is the pipelines with the root folder in the root folder with the ECS and

Kubernetes, we have a pipelines folder and create a new Yaml file under this pipeline.

Which name is shopping API pipeline Yaml.

So we are building from the beginning from the scratch of the shopping API automation pipeline Yaml

file and we are adding these Yaml files into our GitHub repository.

By this way we can track our CI CD flows in the code structure.

It is a best practice.

So after that, now we can already create these successfully.

Generated Yaml file before.

So I am going to copy these Azure pipelines yaml file and paste into my shopping API.

Pipeline yaml file and let's save.

So we verified that these Azure pipelines working perfectly and deploying successfully build and deployment

stage are successfully performed in the last video.

So that's why I have copy and paste into the pipelines because in the last stage the only files should

be stay under the pipelines as per the microservice definitions.

Okay.

After copy and paste the shopping pipeline email, we have base pipeline information now and we have

stage tasks, jobs and all information.

But we are going to refactor these pipeline yml file.

Okay.

So the first refactor is refactoring should be the trigger section.

So we should change the trigger section in order to change any microservice as per as per the microservice

change for our case.

If shopping API is change and after that this pipeline should be triggered.

That means under the shopping folder structure we have a shopping API microservice and shopping client.

So if the shopping client changes, we don't, we should not trigger this shopping API pipeline.

So that's why we can specify that.

Please check the main branch, but also check the shopping API folder structure change.

Don't.

Interesting with the other changes into the GitHub repository.

Okay.

For that purpose we have some several.

Options.

Said that we are going to provide trigger.

Let me replace.

And also I'm saying that we have branches, including the main branch, and also I'm providing the pets

under the branch in the same location, including.

Only my shopping changes.

Shopping slash shopping API.

So we are providing the number.

Now it is.

Giving an error using tops.
...
Okay, let me remove this one also.

Okay.

So let information.

That should be the same level with the branches under the pet.

We have a include definition and the last step we have a.

Folder structure which giving the shopping shopping API.

Okay.

Yes, very good.

So after that, I also would like to give another folder structure, which is the.

Which will be the my manifest file.

Let me make it.

Manifest that shopping.

So as you can see that we have arranged our trigger definition with a set that only shopping chains

could be trigger this pipeline.

And also we have one file under the manifest file and we are storing these informations under the manifest

files, but we are going to remove these manifest files.

So that's why.

Let me correct this folder.

We should check the our source manifest file into the case.

If we change any update in the shopping API, this will also trigger the shopping pipeline.

Okay.

So let me save this trigger.

But this is our first refactoring part change trigger as per the microservices change.

Okay.

So the second thing is the change the artifact folder.

So as you can see that between build and the deployment stage, we have a upload and artwork section.

Basically it provides a connection, communication with the deployment and the build stage.

So for that purpose it is defined the manifest folder.

But this time I'm going to change with the CSS because I have a CSS folder which including all of the

manifest Yaml files for the Kubernetes deployment, so I don't need to create any manifest file.

This is the recommendation for the Microsoft for the manifest file, but we have already developed our

manifest into the CSS folder, right?

So we can update this upload artifact 16 with the CSS folder.

We all manifest files.

It is including the folder x is the top folder or GitHub repository.

Okay.

After that, after this stage, of course we should apply the this change accordingly into the deployment

task.

So in the deployment task, as you can see that there is a manifest.

This should be stay in here, but it is looking for the manifest folder.

I'm going to remove manifest folder and we should use the x folder because in the last video I am totally

whole documentation, copy and paste in the shopping yaml file.

We don't need to copy and paste.

We have already source folder which including all Kubernetes manifest file.

So that's why I am changing my upload artifact operation manifest to x.

Okay, so set that for the shopping API microservices.

In the pipeline of the shopping API, it should be on the move to shopping API Yaml and now we don't

need to store any other pipeline Yaml files.

So let me save my file.

And now I'm going to remove these other pipelines because we are separating the pipelines as per the

microservices.

We don't need the root level pipeline.

I'm deleting that files and also delete the whole manifest file.

We don't need any manifest deployment or service removal.
...
Also, we don't need to generate a pipeline anymore because we have generated these codes from the Azure

DevOps, Azure Pipelines Portal, but anymore we don't need to use these generate HTML file, right,

because we have a existing and working Yaml file in our code structure.

For that purpose I am going to remove these pipeline, but before that let me show you how we can.

Artifact documents between build and deployment stage.

So as you can see that if you open the pipeline and open one of the success run profiles, you will

see the stage in here.

Also jobs stored in here under the stage.

Once we complete it is created one artifact.

If you click the artifact folder, you will see that under the manifest folder we can reach to our shopping

Yaml file.

You can download and also.

See this Xaml file by clicking.

So this is the idea of how we can see pipeline.

Artifacts with building and deploying stage in the pipeline.

Okay.

For now, in my case, I don't need to record this pipeline anymore.

That's why I'm going to delete this file, this pipeline.

Let me.

Give the name and delete the pipeline.

So we have removed the pipeline.

We have removed the Azure pipeline, single file in code base.

We have removed two manifest folder at all in our code structure.

The only file is left.

Pipelines folder.

And I'm creating the shopping API pipeline for the shopping API microservices.

So we have seen how we can refactoring our shopping API pipeline.

And as you can see that we have managed the folder structure of pipelines.

In the next video we will see how to create pipeline with existing Yaml file.
...
**Duration: 8 minutes**
In this video, we are going to create a new pipeline for shopping client microservices.

We have to create two pipelines for two microservices and we trigger these pipelines on related microservices

changes.

This will separate the pipelines for shopping client microservices.

Let's take an action.

So we are going to create a new pipeline for shopping client microservices.

For that purpose, let me close all tops.

And open the pipelines and we are going to create new pipeline for the shopping light.

I would like to copy and paste this file.

You can copy and paste under this file.

And give the name of the pipeline is shopping client email copy from the shopping API because almost

the same.

Let me rename this copy file and give that shopping client pipeline dot Yaml.

Okay.

We have copy all codes from the shopping API and we are going to replace shopping API parts to the shopping

client.

For that purpose, let me find the API and replace with the API.

Is it client?

Okay.

There is for item replace one by one replace.
...
Okay, this should be the lowercase shopping client.

So that's why I'm.

Make it lowercase shopping client.

Also, there is one uppercase.

And this one also should be shopping client.

Okay.

The second one is the shopping API.

If there is a shopping API, let us know that.

Okay.

We should replace with the shopping light.

Let me check this one.

Okay.

Basically we are replacing the API codes with the client project.

Right?

This is the shopping client and we have a shopping client Kubernetes manifest file and there is no API

definition.

Verify that there should be no API definition in the client pipeline yaml file.

Okay.

We have replaced and save this file and we are going to create new pipeline into Azure DevOps portal.

Go to the portal.

And as you can see, that last pipeline also working successfully in our project.

So we are going to create a new pipeline.

And in order to do that, open the pipeline section before creating a new pipeline, I am going to change

the name of this pipeline before we are creating a new pipeline for shopping client microservices.

Let me modify existing pipeline which is belongs to Shopping API.

So click this image and rename this pipeline and give the name of the pipeline is Shopping API Pipeline.

Just click the save.

Okay, we have changed the pipeline name as a shopping API pipeline.

So now we are ready to create new pipeline for the shopping client pipeline.

Create new pipeline for clicking this new pipeline button.

And choose the source code on GitHub.

And of course we are giving our repository.

Repository is ASP.Net.
...
And when we are configuring our pipeline we should check the existing Yaml file.

But of course we should commit our code to the GitHub first.

So let me commit this client yaml file.

Client.

Pipeline.

Let's commit and push your code on to GitHub.

Okay, let me check on GitHub.

Run DevOps and client pipeline came in here.

Okay.

Now we can go further operations.

We have created a new pipeline for shopping client microservice application and using the existing Azure

pipeline Yaml task.

So in the main branch let me check the pipelines.

So now we can see the shopping client pipeline yaml file, select these Yaml file and continue the operations

and you can verify our pipeline definitions.

It is for client application is shopping client and also image repository should be shopping client.

Let me go back and modify this client operation point.

Let me come here.

Find the client.

And check all clients in here.

Okay.

This should be the.

Uppercase, lowercase, and this should be lowercase because in the Azure Container Registry we have

shopping client.

The cover image for now, for that purpose, this should be the lowercase.

This is important for the image repository name.

Okay.

I arranged the codes according to the client microservice.

Again, commit the client.
...
And push this code to the GitHub.

So basically we expect that we have two microservices and we are creating a two pipeline for that purpose.

Let me see in here.

Okay.

Or change reflect to the GitHub.

So now I'm going to create a new pipeline for our shopping client application.

I'm selecting the GitHub and all the repositories.

This is my repository.

When configuring our Yaml, we don't use any predefined template.

We are using the existing pipeline Yaml files and looking for the.

Pipelines, shopping client pipeline.

Let's continue.

So it is good to reviewing your pipeline before running your pipeline.

That's why we can identify this problem of the image repository should be shopping client.

So now everything seems okay.

So let me run this pipeline and we will see the results in the next video.

But let me hit the run button, create the pipeline.

By this way, we have separated the pipelines for only shopping client microservices and it is start
...
**Duration: 19 minutes**
In this video, we are going to see results of the shopping client pipeline.

So let's take an action.

This is our client pipeline refactor.

Commit changes.

So now the stage is started.

And let's follow these locks and steps from the other pipelines.

Now it is built and pushing the shopping client image and push this image to Azure container registry

with a given build ID.

Build ID comes from the Azure pipelines it is generating from them.

And take this build number as a tag number of the our shopping client image.

We have also defined the world context for the Docker file.

So that's why all these Docker steps performing successfully.

Let's wait for a while.

And this will be.

Finish, build and push image to the container registry.

Okay, now I'm pushing.

And push also performed.

Now it is publishing the pipeline artifact.

So which stage is completed?

If you check the artifacts, we can see the folder and it is including both shopping API and also shopping

client HTML file.

And go back to the our run profile and see the deployment stage.

Deployment stage is just started.

Now, in the same time, I would like to create a new terminal.

And see what's happened in the ports is another good way.

As you can see that this is performing rollout deployment, terminating the old shopping clients and

creating a new shopping client pods from the newly tagged and pushing from the Azure Container Registry.

So you can also watch these pods deployment operation with kubectl, get pod dash watch command.

So I think the deployment is done.

You can verify with the.

Azure DevOps dashboard.

Yes, deployments successfully performed.

You can see in here, this is our for the client application.

Just open the pipeline and see the details.

In this stage, you can see the width succeed and deployment succeed.

You can also check the artifacts.

We successfully move this folder with and the deployment stage.

This stage including the shopping client Yaml file.

And if you go back you will see the deployment stage also performed successfully.

So we also seen that how deployment Kubernetes Yaml files running from the Azure pipelines.

In this step this will be using the shopping client Yaml file and deploying the deployment and service

Yaml files manifest file into the cloud.

Azure Kubernetes Services.

We have verified that in here.

As you can see that old pods terminating and new post creating with the kubectl watch command.

If we check again kubectl get pod.

You can see that newly shopping client posts created if we check the details.

Of one of the pots.

Let me verify that.

Which number of the width and take is created.

Just cube Ctl Describe and see the events in the pulled row.

You can see the shopping client 51 is pulling from the Azure Kubernetes services and deploying to the

target environment.

Okay, let's me verify from the our shopping repository on the Azure portal.

So you can refresh this shopping API.

The last API is 50.

So shopping client, if you refresh, you can see the 51 is the or newly deployment from the shopping

client pipelines yaml file.

Okay.

Very good.

Deployment is successfully performed.

So I would like to check the our client application.

In order to do that we can go to the my AKs cluster.

And go to the workloads services and increase.

And you can see the client service in here.

And this is our external IP generated from the Azure Kubernetes services with the Ingress record.

Let's click this URL and see what's happened.

Now, I think we had a problem.

The deployment performed, but in somehow we get a problem for that purpose.

Let me check with the.

Shopping client application.

Let me close this one.

And I'm going to now using troubleshooting.

After our operation for that purpose, we are going to use kubectl, get pod kubectl, describe pod

and kubectl logs.

So let me check this pod.

Okay, I understand the problem, but it takes almost one hour to solve this problem and we can provide

to troubleshooting with kubectl.

Describe kubectl, get pods, kubectl logs.

I'll check all of the things.

You can also apply kubectl get pods and you can use the kubectl describe and see the event list.

Everything seems fine.

And also if we check the logs of the one of the.

Shopping client port.

Also, I was checked for all of them.

Let me write again.

Every pot has the the success looks there is no problem in the lock side.

And also there is no problem with the describing events.

Also looks everything seems fine.

So also I check it for the uh, interactive terminal.

Let me check the interactive terminal.
...
I would like to check environment variables with print and NEA.

And you can check that shopping API URL is also correct.

So there is no network problem.

So what is the problem then?

Let me exit and show that for that cases.

If you check the all of the ports in the Kubernetes logs and describe if you don't find any problem,

we can focus on the our pipeline.

So let me go to the pipeline.

First of all, I would like to see the pipeline Yaml in the pipeline also we can see in the problem.

So you can compare with the API and the client in the client project.

There is no API.

Let me show every API.

Keyword.

We have replaced with the client, so everything seems okay.

So let me check the our pipeline.

This is client shopping client pipeline.

Let me open this pipeline and see the our.

Successfully running profile what we have done together.

We had a client pipeline refactor change and we saw that this pipeline successfully build and deploy.

If you open build stage, we can troubleshooting the one by one all the steps and check the logs.

So this is the check that DevOps looks.

There is no any interesting operation.

So let me open the build and push image container.

So I would like to see this execution with detailing.

So for that purpose, let me come back in here open again.

Yes.

If we check the raw log, we can see that all the logs in the boot section.

Also, I think we can perform this operation in this way, in this level.

But I didn't see.

Okay, Let's come in here.

So I would like to emphasize that I'm going to locate.

The bull tusk.

This is the bull tusk starting operation.

And also, if you can search with the API, I want this row.

If you check the this row, it is basically using the Docker build command.

Okay.

We are expecting this Docker build command, but this is getting the Docker file from the shopping API.

Even we have trigger the.

Or client microservices pipeline.

This will start the shopping API docker file.

So why this happens?

This is very interesting error and hard to find this error and I understand that this error happens

if we put the Docker file with these asterisks are static characters.

That means other pipelines is go and find the first Docker file in our application.

So it is looking to shopping and shopping API in the shopping API.

Once it is detected Docker file, it is not located the shopping client again.

So that means we should specify specific docker file path in our every pipeline Yaml definition.

It is very important and I forgot to change this Docker file path.

As you know that we have only changed the build context, but also we should specify the specific file

path of the Docker file.

So let me change both of the shopping client and the shopping Yaml file.

Close others in the shopping client.

The docker file page should be the.

Pooping and the shopping list.
...
The name of the client should be the.

Or.

Microservice name shopping dot client.

Okay.

So we have specified the.

A custom Docker file location which is intended to our shopping client microservices.

Very good.

It is the same way I am going to change.

Or the curveball part of the shopping pipeline and the pipeline would be the remove the Asterix characters

and set the specific shopping API Docker file.

So by this way, we have defined specific Docker file, both client and the API microservices, and

the Docker file part is very important.

Of course we should also put the build context in our cases.

So now we have to Changeset one for the shopping API, one for the shopping client.

If we push the codes in these two files, this will trigger both of the microservice pipeline.

Let me set that docker file path fixed.

And commit these changes.

Push since on the GitHub.

Let's for a while.

Okay.

So let me verify with the.

Or GitHub.

Yes.

Docker file pet fixed.

And if we check the pipelines.

This will be start one by one.

Let me check again.

Okay, this will not start any pipeline because we forgot to trigger restrictions.

If you open the pipeline, check the trigger section and only change happen in the microservices and

the manifest yaml file, not the pipeline file.

So that's why when we commit our changes about the pipelines, there is no any triggering in the pipeline.

Okay.

We fixed the error but we didn't run pipeline again in order to run our pipeline.

I would like to check in some code in the our shopping client.

Let's go to the shopping client open vs home index page.

And let me say that in here we have to change the cur file path.

So I would like to give the name Docker file path.

Okay.

Once we change the index page, let me see what's happened.
...
When I commit these changes.

And pushed to.

GitHub.

This will trigger or shopping pipeline shopping Client pipeline.
...
Pipeline will be create image from Docker file of the shopping client, not API one.

So okay, let's verify now our commit comes to GitHub.

And let's go to the pipelines.

And our shopping client pipeline is just started.

We can also rename this.

And this is the file text.

Edit Commit to trigger the build and deploy stage.

Let's check now.

Build and push Image Container registry.

So when these jobs are building, I would like to say that it is very important to reading logs from

the pipelines.

So is our case.

We forgot to change Docker files and once I search in here, I found the API project still running in

our client application.

So this is very crucial to check our build and deployment logs in a row lock.

It is very important If you have any problem with the pipelines, the troubleshooting should be the

checking the log files from the pipeline.

Okay.

And we then push succeeded.

So deployment stage will be start, but let me check raw logs for our newly adding.

Project.

And as you can see, there is no shopping API, right.

If you check the client.

That client shopping?

Yes.

This time we can perform the curb operation over the client's Docker file.

So that means this time it will be worked because we are pushing the Docker image from the shopping

client microservices.

Okay, let's see what happens from the deployment side.

Okay.

Also, deployment succeed perform successfully.

Check the pipelines.

Pipelines successfully finished.

If we can verify from the terminal.
...
All pods create successfully.

If you check kubectl get deployment dash or white.

We can see that shopping client deployment will be pulled from the Azure Container Registry with the

54 tag number.

This is our new build number.

We have just run the pipeline.

So if we check the Azure Kubernetes services, let's go to the Azure Kubernetes services.

And check the workloads.

You can see the port's replica sets are created successfully.

If we check the services and the ingress record we have shopping, client application and shopping client

have external IP because the type of the service is a load balancer.

If we click this external IP this time, as you can see that we have successfully worked with the definition

of the Docker file path.

This is what we have done in our commit changes on GitHub.

So this is directly generated building the image and pushing this image to the Azure Container Registry

and after that, Kubernetes apply deployment with our newly created image from the client application.

We have successfully deploying our client application with automation in Azure Pipelines.

So as you can see, that we have successfully automated deployments of our shopping microservices.

In the next video, we will see how we can improve our pipeline experience.
## 10-18 Rename Pipeline and Test Trigger Shopping Pipelines
**Duration: 8 minutes**
In this video, we are going to rename pipeline and test triggering shopping pipelines.

Let's take an action.

Before we start the test, we should rename the pipelines on Azure.

So let me open the pipelines and click the pipelines.

As you can see that we have already changed shopping API pipeline.

So let's click this icon and rename.

Which are popping client.
...
As you can see that now we have two microservices and two pipelines.

You can think about these microservices could be different teams and these teams can be performed any

time in daily deployments.

100 or more deployment in daily time and end user will never affect with using the Azure Kubernetes

services and automation pipeline with Azure Pipelines.

So we have created these infrastructure with both our shopping client and shopping API microservices.

So now we can test our pipelines from the beginning.

We can test one by one.

I'm going to change the API microservice.

After that I'm going to change client microservices and we will watch the pods, how pods will be rollout

deployment and how we can see the zero down zero downtime deployment.

Okay, so let's start with the shopping API pipeline I'm going to open.

Or Visual Studio code and let's close with studio.

Okay, so now let me close others and go to the shopping API.

And we are using the chains on the product context.

Let's open the product context and say that.

And of the end of the course we are going to almost finished our course.

So I am modifying this product with LG, G7 and of course.

So after these chains, let's see what happened.

Okay.

We are going to set API.

Lost pipeline.

Commit these changes.

And push change to the GitHub.

This will be triggering dropping API pipeline on the Azure pipelines.

Okay.

Commit is done.

We can verify the commit.

On GitHub and we can also check the pipelines.

If you refresh the page, we can see that only shopping API pipeline is triggering because we changed

some codes inside of the shopping API microservices.

If you open the pipeline, you will see that build and deployment stage will be handled in our running

profile.

If you open the build, we already see the same steps.

It is going to create a new Docker image with building the image in the shopping API microservices.

And after that, push this Docker image to the Azure Container Registry and publishing this pipeline

artifacts to the folder.

And on the deployment stage, this will be deploying to the Kubernetes.

Okay.

In during this time.

In the meantime, if we are change some codes.

In our shopping client application.

Let me open the index page and let's say version three.

And last run.

Save these changes and.

Go in here.

Version three Client.

Let me commit these changes and push to the GitHub.

This will also triggering the shopping client pipeline.
...
In the meantime, let me start watching the Kubernetes pods.

Okay.

For now, there is no any deployment performed yet.

So if we.

Okay, performing the deployment, if we check the pipelines.

Also client pipeline will be starting.

So shopping API is almost done and shopping client is starting now.

As you can see, it is waiting status because it is working in a queue.

We have only only one agent, so that's why these will be waiting to finish The other shopping client

pipeline is done.

Shopping client is done.

So that's why these.

I mean, shopping is done.

That's why Shopping client is starting from now.

We can also follow the tasks in the run window.

And if we check the our application.

As you can see, that this is performing rollout deployment and Kubernetes, one by one, terminating

the old pods and creating a new pods with the new decrements.

If you can check the homepage if you refresh.

You didn't expect any.

Downtime or any problem when deployment performing.

So deployment is not done for the client application we can follow that deployment stage will be just

start now.

And during this time and user can be continue their operations with the client application.

With the old version of the Docker image.

For now, it is starting to.

Deployment to the Kubernetes.

You can verify that these ports is funding, terminating or creating new ports.

Let's check the end user.

Yes, just perform the deployment.

Updated version three Last run.

As you can see, that end user never effect and we performed the zero downtime deployment with Azure

Kubernetes Services and perform these deployment automating with the Azure pipelines.

You can see the pipelines is almost finished.

Both client and API pipelines successfully finished.

Also, you can check from the Azure dashboard and open the our other container registry and see in which

booth numbers can be adding in the shopping API 55 and shopping client has 56.

So as you can see that we have successfully automated deployments for our shopping microservices with

zero downtime deployment.
## 10-19 Put Azure Pipeline Status Badge into Your Github Project
**Duration: 4 minutes**
In this video, we are going to put Azure pipeline status page into our GitHub project.

Let's take an action.

So we are going to put this page into our GitHub repository.

Open our pipelines, go to the pipeline section and just click this button.

Let me check this button.

Okay.

We should open the one of the pipeline.

Let me open the pipeline.

And if we click this button, you can see the status page.

Just click the search page and.

We are going to see these search page in our GitHub repository.

For that purpose, I will use the sample markdown, so copy the sample markdown and go to GitHub.

Edit our Readme file.

And let me paste these.

With Sotus into my own repository.

So this is basically an image, but image is indicating our latest with result in our Azure pipelines.

So that's why it is important.

For now we put this image for the shopping client pipeline.

Let me commit change and see what's happened.

And this time I think this will not show the well image.

I will fix after that.

But let me go back to pipelines.

And put one image for the shopping API.

Also open the shopping API pipeline, click this page and set the copy the sample markdown.

And you will.

Come to the Readme file.

Put another such as page with status for the shopping API pipelines.

Let's commit these changes.

So now we should.

Get the image from the Azure pipelines.

Okay.

Now, in order to see the correct image, we should have one configuration.

I want to position in the stack overflow.

We can go to the project settings in Azure Pipelines and in the pipeline settings.

We should set the disable anonymous access to batch.

With the disabled section.

Let me open the pipelines and go to the project settings.
...
It will be opened anonymous access to bets.

So I think this will solve our issue.

Let me come back to pipelines if we refresh the page in the GitHub.

Yes, as you can see, that we can finally see the Azure Pipelines image which succeed.

This is the latest build for the pink.

Let me open this one.

This is the shopping client.

And if you come back.

If you open this pipeline, this will also the shopping API.

When you click the badge, it is directly sent to you, redirect to you.

The related pipeline.

When you are running and you can also see the additional details from the build and deployment stage

also logs with reaching these links.

Okay.

In the next video, we are going to clean all other resources.
## 10-20 Clean All AKS and Azure Resources
**Duration: 4 minutes**
In this video, we are going to clean all Azure Kubernetes services and Azure Resources.

Let's take an action.

We are going to clean up resources.

As you know, that Azure subscription is working on pay as you go model.

So it is good to it is good practice to removing usage of the unnecessary times.

So for that purpose, I'm going to delete all Kubernetes items.

Let me check Kubectl get all.

As you know that we have all the image pods, replica sets and services located in our Azure Kubernetes

services because all AKs folder is running and deploying with the pipelines.

Also, we are updating the shopping client and shopping API microservice pods in the pipeline schedules.

Let me close.

In order to delete all Kubernetes resources, we can just use the Kubectl delete and give the file name

AKs.

That's all.

If you hit enter, all Kubernetes resources will be gone.
...
Now there is only cluster IP for the Kubernetes.

There is no item left for our own Azure Kubernetes services.

So this time we only see the Kubernetes resources on the case, but we also need to remove the whole

AKs cluster, also Azure Container Registry on our Azure subscription.

We can also remove all the resources of Azure with deleting Azure Resource Group, which including the

Azure Kubernetes Services, Azure Container Registry and all items for us.

You can also check for the items in the our.

Portal.

Let me come back to Portal.

Right in here.

Portal that Azure.com.

You can see the all the resources under the resource group.

My resource group.

As you can see that we have a case cluster and Azure container registry.

In order to.

Avoid Azure charts, you should clean up unneeded resources.

So when the Kubernetes cluster is no longer needed.

Use the AZ Group delete command in order to remove the resource group, container service and all the

related resources.

So that means if we delete this resource group, this AKs cluster and Shopping ECR will be gone.

I would like to use with the deleting command.

Let me right click copy and paste in here.

AZ group delete giving the name my resource group and no wait.

That means remove from the background.

Just hit the enter.

And this will trigger the delete operation.

And okay, delete is starting.

It takes almost five minutes to reflecting on the portal.

So you can also check the portal.

What's going on in the resources after the five minutes.

I think this will be performed to deleting all resource groups.

By this way, when we delete the resource group, of course, the AKs Azure, Kubernetes Service and

Azure Container Registry will be gone.

Okay.

As you can see that we have removed all items on the cloud.

So it is very fast to creating all the resources, provisioning and cleaning again.
# 第11節：Automate Existing Microservices Reference Application with AKS and Azure DevOps
**Total: 1 lecture | 2 minutes**

## 11-1 Introduction
**Duration: 2 minutes**
In this section we are going to Automate Existing Microservices Reference Application with AKS

and Azure Devops.

The best way to learn is that developing yourself.

So for that reason, I am giving this section as an assignment for you, in order to develop your own

Automate existing micro services architecture, with given the reference application.

Let's take our big picture of the existing microtones architecture.

This is basically an implementation of e-commerce domain in a microservices architecture.

As you can see that, we have developed a reference microservices architecture before this course.

But the deployment part is missing.

So I am expecting to you, developing microservices pipelines in this picture by this learnings of the course.

Before that, let me share the source code of an existing microservices architecture.

You can find the repository in below link, please star and fork the repository and start working on it.

Once you developed you can send me pull request,

I will evaluate and merge the codes if it is expected.

I'm giving you as an assignment, please work on this and develop this feature into your own code repository

and send me your links on Github, I will examine and merge it with our actual repository.
# 第12節：Thanks
